...
class MHA(nn.Module):
    ...
    def __call__(self, x, step, *, train, key_padding_mask=None):
        ...
        value, key_padding_mask, seq_shape = self._process_input(value, key_padding_mask)
        ...
        w_q = self.window_partition(queries, sequence_length, ext_window_size=0)
        w_k = self.window_partition(keys, sequence_length, ext_window_size=self.ext_size)
        w_v = self.window_partition(values, sequence_length, ext_window_size=self.ext_size)

        if self.attn_2d:
            raise NotImplementedError
        else:
            assert sequence_length % self.num_landmarks == 0
            rf_win_size = int(sequence_length // self.num_landmarks)

        rf_w_q = self.window_partition(queries, sequence_length, window_size=rf_win_size, ext_window_size=self.ext_size)
        rf_w_k = self.window_partition(keys, sequence_length, window_size=rf_win_size, ext_window_size=self.ext_size)
        rf_w_v = self.window_partition(values, sequence_length, window_size=rf_win_size, ext_window_size=self.ext_size)
...
x = jnp.arange(batch_size * sequence_length * hidden_dim)
x = x.reshape((batch_size, sequence_length, hidden_dim))
param_key = random.PRNGKey(42)
dropout_key = random.PRNGKey(43)
params = mha.init({'params': param_key, 'dropout': dropout_key}, [x, x, x], step=1, train=True)
attention_mat = mha.apply(params, [x, x, x], step=0, train=True, rngs={'dropout': dropout_key})
print(attention_mat)
